...
description: "Free browser-based data analysis tools. Profile datasets, calculate column statistics, analyze JSON, & find duplicate records 100% locally in browser."
permalink: /analyze/
is_category: true
...
<div class="category-page" style="max-width: 1240px; margin: 0 auto; padding: 2.5rem 1.5rem;">
  ...
  <header style="margin-bottom: 2.5rem; text-align: center;">
    ...
    <div style="max-width: 540px; margin: 0 auto 1rem; position: relative;">
      <input type="text" id="category-search-input" placeholder="Search within Analyze tools..." 
    ...
    {% assign category_tools = site.data.tools | where: "category", "Analyze" %}
    <div style="font-size: 0.9rem; font-weight: 600; color: #16a34a; background: #f0fdf4; display: inline-block; padding: 0.35rem 1rem; border-radius: 9999px;">
    ...
    <div id="category-tools-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 1.5rem;">
      {% for tool in category_tools %}
        <div class="category-tool-card" data-title="{{ tool.title | downcase }}" data-keywords="{{ tool.keywords | join: ' ' | downcase }}" data-desc="{{ tool.description | downcase }}"
          ...
          <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 0.75rem;">
            ...
            <span style="font-size: 0.75rem; font-weight: 600; padding: 0.2rem 0.6rem; background: #f0fdf4; color: #16a34a; border-radius: 9999px;">
              Analyze
            </span>
          </div>
      ...
      {% else %}
        <div style="grid-column: 1 / -1; text-align: center; padding: 3rem; background: #f8fafc; border-radius: 12px; border: 1px dashed #cbd5e1;">
          <p style="color: #64748b; font-size: 1.05rem;">No tools currently registered under <strong>Analyze</strong>. Tools added to <code>_data/tools.yml</code> with <code>category: Analyze</code> will automatically display here.</p>
        </div>
      {% endfor %}
    </div>
  </section>
  ...
  <section style="max-width: 850px; margin: 0 auto;">
    <h2 style="font-size: 1.5rem; font-weight: 700; color: #0f172a; margin-bottom: 1.25rem; text-align: center;">Analyzation FAQ</h2>
    <div style="display: flex; flex-direction: column; gap: 1rem;">
      <details style="background: #ffffff; border: 1px solid #e2e8f0; border-radius: 10px; padding: 1rem; cursor: pointer;">
        <summary style="font-weight: 600; color: #0f172a;">What are Analyze Tools?</summary>
        <p style="margin-top: 0.5rem; color: #475569; font-size: 0.95rem;">Analyze tools compute data summaries, distributions, statistics, and column metrics locally in your web browser.</p>
      </details>
    </div>
  </section>
